export class GetKeys {
  ...
  public static getIndexs(
    dimensionMetadataId: number,
    stateMetadataId: number
  ) {
    let indexDimension = this.source
      .map(function(item) {
        return item.metadataId
      })
      .indexOf(dimensionMetadataId)
    let indexState = this.source[indexDimension].states
      .map(function(item) {
        return item.metadataId
      })
      .indexOf(stateMetadataId)
    return {
