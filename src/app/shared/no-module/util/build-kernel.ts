...
import { Kernel, Alpha, State, Checkpoint } from '../models/project-kernel'
  ...
  public static build(alphas: any[]): Kernel {
    if (!!alphas) {
      ...
      for (let alphaRecord of alphas) {
        let alphaMetadata = ALPHAS.find(
          alpha => alpha.identifier === alphaRecord.metadataId
        )
        let dimension = new Alpha(
          alphaMetadata,
          alphaRecord.isTouched,
          alphaRecord.metadataId
        )
        ...
        for (let stateRecord of alphaRecord.states) {
          ...
          let state = new State(
            stateMetadata,
            stateRecord.isAchieved,
            stateRecord.isWorking
          )
          for (let checkpointRecord of stateRecord.checklist) {
            let checkpointMetadata = stateMetadata.getCheckPoint(
              checkpointRecord.metadataId
            )
            let check = new Checkpoint(
              checkpointMetadata,
              checkpointRecord.isAchieved
            )
